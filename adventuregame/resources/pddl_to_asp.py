...
def action_to_asp(action_def: dict, trait_dict: dict = None):  # type: ignore[assignment]
    ...
    asp_potential_action = asp_potential_action.replace(
        "ACTION_TYPE", processed_action_pddl["action_name"]
    )
    mutable_asp_strings = [f"{mutable}_t(TURN,THING)" for mutable in important_precon_mutables]
    asp_potential_action = asp_potential_action.replace(
        "PRECON_FACTS", ",".join(mutability_asp_strings + mutable_asp_strings)
    )
    # collect:
    ...
    effect_add_asp_strings = [
        f"{pred}_t(TURN+1,THING)"
        for pred in [effect_pred["predicate"] for effect_pred in effect_add_predicates]
    ]
    ...
    if trait_dict[action_mutabilities[0]]["interaction"] == "irreversible":
        ...
        asp_next_turn_irreversible = asp_next_turn_irreversible.replace(
            "MUTABLE_FACT", f"{trait_dict[action_mutabilities[0]]['mutable_states'][-1]}"
        )
