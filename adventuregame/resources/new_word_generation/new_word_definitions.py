...
def new_word_rooms_replace(
    source_definition_file_path: str,
    num_replace: int = 0,
    last_new_words_idx: int = 0,
    seed: int = 42,
):
    ...
    for def_idx in def_idx_to_replace:
        ...
        cur_def["repr_str"] = new_words_source[list(new_words_source.keys())[new_word_idx]]["pos"][
            "NN"
        ]

        old_type_name = cur_def["type_name"]
        cur_def["type_name"] = new_words_source[list(new_words_source.keys())[new_word_idx]]["pos"][
            "NN"
        ]
...
def new_word_rooms_create(
    num_rooms_created: int = 4,
    min_connections: int = 1,
    max_connections: int = 4,
    max_exit_targets: int = 4,
    last_new_words_idx: int = 0,
    seed: int = 42,
):
    ...
    for def_idx in range(num_rooms_created):
        ...
        new_room_type_name = new_words_source[list(new_words_source.keys())[new_word_idx]]["pos"][
            "NN"
        ]
        new_word_idx += 1
    ...
    for new_room_type_def in new_room_definitions:
        ...
        new_exit_targets = rng.choice(
            new_room_type_names_culled, size=rng.integers(1, max_exit_targets), replace=False
        )
...
def new_word_entities_create(
    room_definitions: list,
    num_entities_created: int = 10,
    min_std_locations: int = 1,
    max_std_locations: int = 3,
    add_traits: bool = False,
    premade_traits: list = [],
    limited_trait_pool: int = 0,
    min_traits: int = 0,
    max_traits: int = 3,
    add_adjectives: bool = False,
    premade_adjectives: list = [],
    limited_adjective_pool: int = 0,
    ...
    if add_traits and not premade_traits and limited_trait_pool:
        for new_word in range(limited_trait_pool):
            ...
            trait_pool.append(
                f"{new_words_source[list(new_words_source.keys())[new_word_idx]]['pos']['JJ']}_able"
            )
            new_word_idx += 1
    ...
    if add_adjectives and not premade_adjectives and limited_adjective_pool:
        for new_word in range(limited_adjective_pool):
            adjective_pool.append(
                new_words_source[list(new_words_source.keys())[new_word_idx]]["pos"]["JJ"]
            )
            new_word_idx += 1
    ...
    for def_idx in range(num_entities_created):
        ...
        new_entity_type_name = new_words_source[list(new_words_source.keys())[new_word_idx]]["pos"][
            "NN"
        ]
        new_word_idx += 1
        ...
        if add_traits:
            ...
            if limited_trait_pool == 0 and not premade_traits:
                ...
                new_entity_type_dict["traits"] += rng.choice(
                    trait_pool, size=rng.integers(min_traits, max_traits), replace=False
                ).tolist()
        # adjectives:
        if add_adjectives:
            new_entity_type_dict["possible_adjs"] = list()
            if limited_adjective_pool == 0 and not premade_adjectives:
                for add_adj_idx in range(
                    min_adjectives, rng.integers(min_adjectives, max_adjectives)
                ):
                    taken_new_word = new_words_source[list(new_words_source.keys())[new_word_idx]][
                        "pos"
                    ]["JJ"]
                    new_word_idx += 1
                ...
                new_entity_type_dict["possible_adjs"] += rng.choice(
                    adjective_pool, size=rng.integers(min_adjectives, max_adjectives), replace=False
                ).tolist()
        # standard locations:
        new_entity_type_dict["standard_locations"] = rng.choice(
            [room_def["type_name"] for room_def in room_definitions],
...
def new_word_actions_replace(
    source_definition_file_path: str,
    num_replace: int = 0,
    is_like_explanations: bool = False,
    last_new_words_idx: int = 0,
    seed: int = 42,
):
...
def new_word_actions_create(
    entity_definitions: list,
    num_actions_created: int = 6,
    trait_pool: list = [],
    ...
    for trait, trait_features in trait_dict.items():
        if (
            trait_features["interaction"] == "irreversible"
            and created_actions_count < num_actions_created
        ):
            ...
            fail_parameters_trait_type = "The {{ e }} is not MUTABILITY_TRAIT.".replace(
                "MUTABILITY_TRAIT", trait_dashed
            )
            fail_parameters = [
            ...
            if trait_features["mutable_set_type"] == "singular":
                ...
                fail_precondition_entity_state = "The {{ e }} is not MUTABLE_STATE.".replace(
                    "MUTABLE_STATE", trait_features["mutable_states"][0]
                )
                fail_precondition = [
                    [
                        "You are not where you are! (This should not occur.)",
            ...
            # SUCCESS FEEDBACK
            if trait_features["mutable_set_type"] == "singular":
                success_feedback = "The {{ e }} is now MUTABLE_STATE_POST.".replace(
                    "MUTABLE_STATE_POST", trait_features["mutable_states"][0]
                )
            elif trait_features["mutable_set_type"] == "paired":
                success_feedback = "The {{ e }} is now MUTABLE_STATE_POST.".replace(
                    "MUTABLE_STATE_POST", trait_features["mutable_states"][1]
                )
        ...
        if (
            trait_features["interaction"] == "binary"
            and created_actions_count < num_actions_created
        ):
            ...
            # precondition failures:
            fail_precondition_entity_state = "The {{ e }} is not MUTABLE_STATE.".replace(
                "MUTABLE_STATE", trait_features["mutable_states"][1]
            )
            fail_precondition = [
                ["You are not where you are! (This should not occur.)", "world_state_discrepancy"],
                ["You can't see a {{ e }} here.", "entity_not_accessible"],
        ...
        if (
            trait_features["interaction"] == "trinary"
            and created_actions_count < num_actions_created
        ):
            ...
            # SUCCESS FEEDBACK
            success_feedback = "The {{ e }} is now MUTABLE_STATE_POST.".replace(
                "MUTABLE_STATE_POST", trait_features["mutable_states"][2]
            )
            new_action["success_feedback"] = success_feedback
            ...
            # precondition failures:
            fail_precondition_entity_state = "The {{ e }} is not MUTABLE_STATE.".replace(
                "MUTABLE_STATE", trait_features["mutable_states"][2]
            )
            fail_precondition = [
                ["You are not where you are! (This should not occur.)", "world_state_discrepancy"],
                ["You can't see a {{ e }} here.", "entity_not_accessible"],
...
def process_to_pddl_domain(
    domain_name: str, room_definitions: list, entity_definitions: list, trait_dict: dict
):
...
def create_new_words_definitions_set(
    initial_new_word_idx: int = 0, seed: int = 42, verbose: bool = False
):
    ...
    if verbose:
        num_defs_created = (
            len(new_room_definitions) + len(new_entity_definitions) + len(new_action_definitions)
        )
    ...
    return (
        new_room_definitions,
        new_entity_definitions,
        new_action_definitions,
        new_domain_definition,
        trait_dict,
        last_new_word_idx,
    )


def replace_new_words_definitions_set(
    initial_new_word_idx: int = 0,
    seed: int = 42,
    verbose: bool = False,
    ...
    new_room_definitions, last_new_word_idx, rooms_replaced = new_word_rooms_replace(
        source_definition_file_path=room_definition_source,
        num_replace=room_replace_n,
        last_new_words_idx=initial_new_word_idx,
        seed=seed,
    )

    new_entity_definitions, last_new_word_idx, entities_replaced = new_word_entities_replace(
        source_definition_file_path=entity_definition_source,
        num_replace=entity_replace_n,
        last_new_words_idx=last_new_word_idx,
        seed=seed,
    )

    new_action_definitions, last_new_word_idx, actions_replaced = new_word_actions_replace(
        source_definition_file_path=action_definition_source,
        num_replace=action_replace_n,
        last_new_words_idx=last_new_word_idx,
        seed=seed,
    )
