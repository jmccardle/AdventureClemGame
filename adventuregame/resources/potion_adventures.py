...
def generate_potion_recipe(
    domain_def: dict,
    entity_defs: dict,
    tool_categories: tuple = ("stirrer", "wand"),
    n_ingredients: int = 4,
    n_tools: int = 2,
    n_steps: int = 6,
    ...
    steps.append(
        {"step_type": "add_ingredient", "entity_type": ingredients[0], "instruction": str()}
    )
    if always_bucket:
        steps[0][
            "instruction"
        ] = f"pour {entity_defs[steps[0]['entity_type']]['repr_str']} into your cauldron"
    ...
    for step in range(n_steps - 1):
        ...
        for entity_category in domain_def["types"]:
            if (
                step_entity in domain_def["types"][entity_category]
                and entity_category in viable_entity_categories
            ):
                ...
                if entity_category == "stirrer":
                    step_instruction = (
                        f"{rng.choice(['stir', 'agitate', 'mix'])} the liquid using a "
                        f"{entity_defs[step_entity]['repr_str']}"
                    )
                elif entity_category == "wand":
                    step_instruction = (
                        f"{rng.choice(['wave', 'wiggle', 'swirl', 'use'])} a "
                        f"{entity_defs[step_entity]['repr_str']} "
    ...
    ingredients_text: str = (
        f"Ingredients: {ingredients_repr_strs[0].capitalize()}, "
...
def create_potion_recipe_events(
    potion_recipe: dict,
    domain_def: dict,
    entity_defs: dict,
    tool_categories: tuple = ("stirrer", "wand"),
    ...
    for step_idx, step in enumerate(potion_recipe["steps"][step_start:]):
        if verbose:
            logger.debug("Step: %s", step)
        step_event_dict: dict = {
            "type_name": f"potion_step_{step_idx+1}",
            "pddl": "",
            "event_feedback": "",
            "asp": "",
        }
        ...
        if step["step_type"] == "add_ingredient":
            ...
            if step_idx == 0:  # first step
                ...
                asp_event_trigger = (
                    "event_t(TURN,EVENT_TYPE,ROOM) :- turn(TURN), room(ROOM,_), PRECON_FACTS."
                )
                asp_event_trigger = asp_event_trigger.replace(
                    "EVENT_TYPE", f"potion_step_{step_idx+1}"
                )
                precon_facts = (
                    f"at_t(TURN,{prior_ingredient}1,ROOM), at_t(TURN,{current_entity}1,ROOM), at_t(TURN,cauldron1,ROOM),"
                    f"in_t(TURN,{prior_ingredient}1,cauldron1), in_t(TURN,{current_entity}1,cauldron1)"
                )
                asp_event_trigger = asp_event_trigger.replace("PRECON_FACTS", precon_facts)
                if verbose:
                    logger.debug("ASP event trigger: %s", asp_event_trigger)
                asp_rules.append(asp_event_trigger)
                # event effects ASP:
                # added facts:
                if verbose:
                    logger.debug("add facts:")
                # asp_next_turn_add = "MUTABLE_FACTS :- event_t(TURN,EVENT_TYPE,ROOM)."
                asp_next_turn_add = " :- event_t(TURN,EVENT_TYPE,ROOM)."
                asp_next_turn_add = asp_next_turn_add.replace(
                    "EVENT_TYPE", f"potion_step_{step_idx+1}"
                )
                effect_add_asp_list = [
                    f"type(liquid1,liquid)",
                ...
                effect_add_asp_rules_list = [
                    f"type(liquid1,liquid){asp_next_turn_add}",
                ...
                if verbose:
                    logger.debug("ASP next turn add: %s", asp_next_turn_add)
                asp_rules.append(asp_next_turn_add)
                # persist added facts in ASP:
                asp_persistence_facts = [
                    fact_str for fact_str in effect_add_asp_list if "_t" in fact_str
                ]
                if verbose:
                    logger.debug("asp_persistence_facts: %s", asp_persistence_facts)
                for asp_persistence_fact in asp_persistence_facts:
                    ...
                    asp_next_turn_persist = asp_next_turn_persist.replace(
                        "PERSISTENT_FACT", asp_persistence_fact.replace("TURN", "TURN+1")
                    )
                    asp_rules.append(asp_next_turn_persist)
                # removed facts:
                if verbose:
                    logger.debug("remove facts:")
                effect_sub_predicates = [
                    f"at_t(TURN,{prior_ingredient}1,ROOM)",
                ...
                for sub_pred in effect_sub_predicates:
                    ...
                    asp_next_turn_sub = asp_next_turn_sub.replace(
                        "EVENT_TYPE", f"potion_step_{step_idx + 1}"
                    )
                    ...
                    asp_next_turn_sub = asp_next_turn_sub.replace(
                        "MUTABLE_FACT_AFTER", effect_sub_asp_string
                    )
                    if verbose:
                        logger.debug("ASP next turn sub: %s", asp_next_turn_sub)
                ...
                effect_add_asp_list = [
                    f"type(potion1,potion)",
                    f"at_t(TURN,potion1,ROOM)",
                    f"in_t(TURN,potion1,cauldron1)",
                    f"accessible_t(TURN,potion1)",
                ]
                effect_add_asp_rules_list = [
                    f"type(potion1,potion){asp_next_turn_add}",
                ...
                for asp_persistence_fact in asp_persistence_facts:
                    ...
                    asp_next_turn_persist = (
                        f"PERSISTENT_FACT :- {asp_persistence_fact}, turn(TURN), room(ROOM,_)."
                    )
                    asp_next_turn_persist = asp_next_turn_persist.replace(
                        "PERSISTENT_FACT", asp_persistence_fact.replace("TURN", "TURN+1")
                    )
                ...
                if verbose:
                    logger.debug("remove facts:")
                effect_sub_predicates = [
                    f"at_t(TURN,liquid{step_idx},ROOM)",
                ...
                effect_add_asp_rules_list = [
                    f"type(liquid{step_idx + 1},liquid){asp_next_turn_add}",
            ...
            if verbose:
                logger.debug("use_tool step, %s", current_entity)
            ...
            if verbose:
                logger.debug("%s applies %s", current_entity, applied_predicate)
...
if __name__ == "__main__":
    domain_def = {
        ...
        "functions": [
            {
                "function_def_predicate": "itemcount",
                "function_def_variable": {"variable": "i"},
    ...
    entity_defs = {
        "player": {"repr_str": "you", "hidden": True, "traits": []},
        "inventory": {
            "repr_str": "inventory",
            "hidden": True,
            "container": True,
        ...
        "wardrobe": {
            ...
            "openable": True,
            "traits": ["container", "openable", "receptacle"],
            "possible_adjs": ["wooden", "large", "fancy"],
        ...
        "chair": {
            ...
            "support": True,
            "traits": ["support", "receptacle"],
            "possible_adjs": ["comfy", "wooden", "padded"],
            "standard_locations": ["readingroom"],
        },
        "bed": {
            "repr_str": "bed",
            "support": True,
            "traits": ["support", "receptacle"],
            "possible_adjs": ["comfy", "wooden"],
            "standard_locations": ["bedroom"],
        },
        "couch": {
            "repr_str": "couch",
        ...
        "broom": {
            ...
            "movable": True,
            "supported": True,
            "traits": ["takeable", "movable", "needs_support"],
            "standard_locations": ["broomcloset"],
        },
        "mop": {
            "repr_str": "mop",
            "takeable": True,
        ...
        "toad": {
            ...
            "movable": True,
            "supported": True,
            "possible_adjs": ["warty", "hypnotic"],
            "traits": ["takeable", "movable", "needs_support"],
            "standard_locations": ["bedroom", "kitchen", "pantry"],
        },
        "greenbeetle": {
            "repr_str": "green beetle",
            "takeable": True,
            "movable": True,
            "supported": True,
        ...
        "pricklypear": {
            "repr_str": "prickly pear",
            "takeable": True,
            "movable": True,
            "supported": True,
            "traits": ["takeable", "movable", "needs_support", "ingredient"],
            "possible_adjs": ["large", "small"],
            "standard_locations": ["herbarium"],
        },
        "waterbucket": {
            "repr_str": "bucket of water",
            "takeable": True,
            "movable": True,
            "supported": True,
            "traits": ["takeable", "movable", "needs_support", "ingredient"],
        ...
        "water": {
            ...
            "movable": True,
            "supported": True,
            "traits": ["needs_support", "ingredient"],
            "possible_adjs": ["clear", "fresh"],
            "standard_locations": [],
        },
        "plasmbucket": {
            "repr_str": "bucket of ectoplasm",
            "takeable": True,
            "movable": True,
            "supported": True,
            "traits": ["takeable", "movable", "needs_support", "ingredient"],
            "possible_adjs": ["large", "small"],
            "standard_locations": ["storage", "garden"],
        ...
        "cauldron": {
            ...
            "container": True,
            "traits": ["container", "receptacle"],
            "possible_adjs": ["copper", "iron", "earthen"],
            "standard_locations": ["kitchen"],
        },
        "ladle": {
            "repr_str": "ladle",
        ...
        "firewand": {
            ...
            "possible_adjs": ["rusty", "copper"],
            "traits": ["takeable", "movable", "needs_support", "tool"],
            "standard_locations": ["workshop"],
            "applied_predicate": "hot",
        },
        "icewand": {
            "repr_str": "ice wand",
            "takeable": True,
        ...
        "fairywand": {
            ...
            "traits": ["takeable", "movable", "needs_support", "tool"],
            "standard_locations": ["workshop"],
            "applied_predicate": "glittery",
        },
        "zulpowand": {
            "repr_str": "zulpo's wand",
            "takeable": True,
            "movable": True,
            "supported": True,
            "possible_adjs": ["rusty", "copper"],
